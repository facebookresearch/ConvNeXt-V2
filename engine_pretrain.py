...
def train_one_epoch_clip(model: torch.nn.Module,
    ...
    optimizer.zero_grad()
    #for data_iter_step, (samples, images) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):    
    for data_iter_step, (batch, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        samples, images = batch #vit, clip
         # we use a per iteration (instead of per epoch) lr scheduler
        if data_iter_step % update_freq == 0:
           utils.adjust_learning_rate(optimizer, data_iter_step / len(data_loader) + epoch, args)
        ...
        loss, _, _ = model(samples, clip_features, labels, args.mask_ratio )
